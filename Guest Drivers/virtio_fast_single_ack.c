...
static unsigned int features[] = {VIRTIO_FEATURES_VQ_SINGLEACK};

static ssize_t proc_read(struct file *file, char *buffer, size_t count, loff_t *offp)
{
	DEBUG printk(KERN_INFO"virtio_fast_single_ack: proc_read");

	if(!virt_fast->dataPresent)
	{
		if(!virt_fast->msgSent)
		{
			DEBUG printk(KERN_INFO"virtio_fast_single_ack proc_read: No data present");
			if(copy_to_user(buffer, "No data present\n",16))
			{
				printk(KERN_ERR"virtio_fast_single_ack proc_read: Error copying message to user buffer");
				return -EFAULT;
			}
	...
	else
	{
	    ...
	    {
	    	printk(KERN_ERR"virtio_fast_single_ack proc_read: Error getting memory for inbuffer!");
	    	return ENOMEM;
	    }

	    sg_init_one(&sl_in,virt_fast->vq_in_buf,virt_fast->lastSentBytes);
	    DEBUG printk(KERN_INFO"virtio_fast_single_ack proc_read: scatterlist done\n");

        if(virtqueue_add_inbuf(virt_fast->queue,&sl_in,1,virt_fast->vq_in_buf,GFP_KERNEL) < 0)
	    {
	    	printk(KERN_ERR"virtio_fast_single_ack proc_read: Error adding inbuffer to queue!\n");
            kfree(virt_fast->vq_in_buf);
	    	return -1;
	    }
	    if(!virtqueue_kick(virt_fast->queue))
	    {
	    	printk(KERN_ERR"virtio_fast_single_ack proc_read: Kick failed\n");
            kfree(virt_fast->vq_in_buf);
	    	return -1;
	    }
	    DEBUG printk(KERN_INFO"virtio_fast_single_ack proc_read: Kick successful\n");
...
static ssize_t proc_write(struct file *file, const char *buffer, size_t bufSize, loff_t *offp)
{
	...
	DEBUG printk(KERN_INFO"virtio_fast_single_ack: proc_write");
	...
	{
		printk(KERN_ERR"virtio_fast_single_ack proc_write: Error getting memory for outbuffer!");
		return ENOMEM;
	}

	if(copy_from_user(virt_fast->vq_out_buf,buffer,bufSize))
	{
        printk(KERN_ERR"virtio_fast_single_ack proc_write: Error copying data from user space!");
        kfree(virt_fast->vq_out_buf);
		return -1;
	}

	sg_init_one(&sl_out, virt_fast->vq_out_buf,bufSize);
	DEBUG printk(KERN_INFO"virtio_fast_single_ack proc_write: scatterlist done\n");

	if(virtqueue_add_outbuf(virt_fast->queue,&sl_out,1,virt_fast->vq_out_buf,GFP_KERNEL) < 0)
	{
		printk(KERN_ERR"virtio_fast_single_ack proc_write: Error adding outbuffer to queue!\n");
        kfree(virt_fast->vq_out_buf);
		return -1;
	}

	DEBUG printk(KERN_INFO"virtio_fast_single_ack proc_write: buffer attached\n");

	if(!virtqueue_kick(virt_fast->queue))
	{
		printk(KERN_ERR"virtio_fast_single_ack proc_write: Kick failed\n");
        kfree(virt_fast->vq_out_buf);
		return -1;
	}
	DEBUG printk(KERN_INFO"virtio_fast_single_ack proc_write: Kick successful\n");
...
{
    DEBUG printk(KERN_INFO"virtio_fast_single_ack: create_proc\n");
	virt_fast->procdirectory = proc_mkdir("virtio",NULL);
	proc_create_data("virtio_fast_single_ack",0660,virt_fast->procdirectory,&procFunc,NULL);
}

void virt_fast_remove(struct virtio_device *device)
{
	DEBUG printk(KERN_INFO"virtio_fast_single_ack: remove\n");
}

static void vq_cb(struct virtqueue *vq)
{
	int size;
	DEBUG printk(KERN_INFO"virtio_fast_single_ack: vqh callback\n");

	virt_fast->lastReceivedMsg = virtqueue_get_buf(vq,&size);

	if(size == 0)
	{
		//Host acknowledged last message
        kfree(virt_fast->vq_out_buf);
		return;
	}
	else if(size != virt_fast->lastSentBytes)
	{
		DEBUG printk(KERN_ERR"virtio_fast_single_ack: Error: Size of received data does not match size of sent data, size: %d, lastSentBytes: %zu",size,virt_fast->lastSentBytes);
		return;
	}

    complete(&virt_fast->cb_done);

	DEBUG printk(KERN_INFO"virtio_fast_single_ack: received: %s bytes: %d\n",virt_fast->lastReceivedMsg,size);
}
...
int virt_fast_probe(struct virtio_device *device)
{
	...
	DEBUG printk(KERN_INFO"virtio_fast_single_ack: probe\n");

	//Allocate memory for this driver
	virt_fast = kzalloc(sizeof(struct virt_fast_driv_dat),GFP_KERNEL);
	if(virt_fast == NULL)
	{
		printk(KERN_ERR"virtio_fast_single_ack probe: Error allocating memory for driver structure\n");
		return ENOMEM;
	}
	...
	{
		printk(KERN_ERR"virtio_fast_single_ack probe: error no config\n");
		return -EINVAL;
	}

	//Reset the virtio device
	DEBUG printk(KERN_INFO"virtio_fast_single_ack: reset device\n");
	...
	//Check the features
	if(virtio_has_feature(virt_fast->dev,VIRTIO_FEATURES_VQ_SINGLEACK))
	{
		DEBUG printk(KERN_INFO"virtio_fast_single_ack probe: Single queue available");
	}
	...
	if(ret)
	{
		printk(KERN_ERR"virtio_fast_single_ack: probe error getting vqs\n");
		return ret;
	}

	virt_fast->queue	 = vqs[0];

	DEBUG printk(KERN_INFO"virtio_fast_single_ack probe: got vqs");
...
{
	.driver.name = "virtio-fast-single-ack",
...
int __init hw_init(void)
{
    ...
    DEBUG printk(KERN_INFO"virtio_fast_single_ack: started\n");

	//Register driver
	if(register_virtio_driver(&virt_fast_driv))
	{
		printk(KERN_ERR"Register of virtio_fast_single_ack failed\n");
		return -1;
	}
...
{
	DEBUG printk(KERN_INFO"virtio_fast_single_ack: stopped\n");

	//Remove the proc file
	remove_proc_entry("virtio_fast_single_ack", virt_fast->procdirectory);
